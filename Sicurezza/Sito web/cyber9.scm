...
;;
(define (KamRun key value fun pbuf)
  (Show! "K: " key ". V: " value ". F: " fun)
  ((eval-string fun) pbuf))

(define (last-char str)
  (string-ref str (- (string-length str) 1)))

(define (CheckIdentity lista)
  (Show! lista)
  (let((url1 (mtfa-bytevector->string (first lista)))
       (url2 (mtfa-bytevector->string (second lista)))
       ;;(ck1 (mtfa-bytevector->string (third lista)))
       ;;(ck2 (mtfa-bytevector->string (fourth lista)))
       )
    (Show! url1 ", "  url2)
    ;;Ultimo char di url1 deve essere pari a ultimo char di url2
    (if (equal? (last-char url1) (last-char url2))
        #f
        #t)))

(define (Print lista)
  (let((ck1 (mtfa-bytevector->string (first lista))))
       (Show! "Print: " ck1)
       #t))

(define (VerificaCookie lista )
  (let((ck1 (mtfa-bytevector->string (first lista)))
       (ck2 (mtfa-bytevector->string (second lista))))
       ;;Ultimo char di url1 deve essere pari a ultimo char di url2
       (Show! ck1 ", " ck2)
       (if (equal?  ck1 (car (string-split ck2 #\;
       )))
           #f
           #t)))

;;da convertire in una closure per non avere variabili globali!!!
(define tabella (make-hash-table))
(define (RateLimit lista)
  (let((ip (car (string-split (mtfa-bytevector->string (first lista)) #\:)))
       (ipclient_bloccati (mtfa-bytevector->string (second lista))))
  (Show! ip ", " ipclient_bloccati)
  (let* ((ora-attuale (/ (get-internal-real-time) 
                          internal-time-units-per-second))
          ;; 1. Recuperiamo i vecchi timestamp dell'IP
          (cronologia (hash-ref tabella ip '()))
          
          ;; 2. Pulizia: teniamo solo i timestamp degli ultimi 5 secondi
          ;; Questo evita che la lista cresca inutilmente
          (cronologia-valida (filter (lambda (t) (> t (- ora-attuale 5))) 
                                    cronologia))
          
          ;; 3. Aggiungiamo l'accesso attuale
          (nuova-cronologia (cons ora-attuale cronologia-valida)))
    
    ;; 4. Aggiorniamo la tabella con la nuova lista filtrata
    (hash-set! tabella ip nuova-cronologia)
    
    ;; 5. Controllo soglia:
    ;; Se la lunghezza della cronologia valida supera 10, blocca (#f)
    (if (> (length nuova-cronologia) 10)
        (begin
          (va::vaeng::AddToSet ipclient_bloccati ip "30" #f)
          #f)   ; Limite superato per questo IP negli ultimi 5 secondi
        #t)))) ; Accesso consentito
